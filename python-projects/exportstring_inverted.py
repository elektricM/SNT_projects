...
for y in range(height):
    ...
    while x < width:
        color_count = 0
        while color_count<25 and x + color_count < width and img.getpixel((x + color_count, y)) == img.getpixel((x, y)):
            color_count += 1
        if color_count > 1:
            color_string += chr(color_count+65) + color_letters[img.getpixel((x, y))]
            x += color_count
        else:
